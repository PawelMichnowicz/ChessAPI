...
async def send_msg_to_opponent(websocket, game, text):
    for user in connected_users[game.id]:
        if user["websocket"] != websocket:
            await user["websocket"].send(text)
...
async def main(websocket, game, username):
    ...
    async for message in websocket:
        ...
        if message["type"] == "give up":
        ...
        elif message["type"] == "draw proposal":
            await send_msg_to_opponent("draw offer")

        elif message["type"] == "draw response" and message["data"] == "Y":
            ...
            try:
                start_field, end_field = (message["data"]).split(":")
            ...
            print(message)
            raise Exception("Co to za message?")
